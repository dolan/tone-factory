...
var defaultDictionary2 = index_default7.all;
var defaultVoiceLeading = index_default6.topNoteDiff;
...
function search(chord2, range3 = defaultRange, dictionary3 = index_default7.triads) {
  const [tonic, symbol] = chord_default.tokenize(chord2);
  const sets = index_default7.lookup(symbol, dictionary3);
  if (!sets) {
    return [];
  }
  const voicings = sets.map((intervals2) => intervals2.split(" "));
  const notesInRange = index_default4.chromatic(range3);
  return voicings.reduce((voiced, voicing) => {
    ...
    const bottomPitchClass = index_default2.transpose(tonic, voicing[0]);
    const starts = notesInRange.filter((note2) => index_default2.chroma(note2) === index_default2.chroma(bottomPitchClass)).filter(
      (note2) => (index_default2.midi(
        index_default2.transpose(
          note2,
          relativeIntervals[relativeIntervals.length - 1]
        )
      ) || 0) <= (index_default2.midi(range3[1]) || 0)
    ).map((note2) => index_default2.enharmonic(note2, bottomPitchClass));
    const notes4 = starts.map(
      (start) => relativeIntervals.map((interval2) => index_default2.transpose(start, interval2))
    );
